...
interface ConsoleProps {
    onNextDay?: () => void;
    reportText?: string;
    // ⭕ 부모 지휘소에서 보내준 차트 갱신 스위치 통로
    onSimulationAdvance?: (newCandleFromServer: any) => void;
    // 🎯 부모가 관리하는 지표 상태와 변경 함수 (옵셔널 바인딩 방어 포함)
    selectedIndicators?: string[];
    setSelectedIndicators?: React.Dispatch<React.SetStateAction<string[]>>;
}

// 🛡️ [전역 방어 상수]: Props 전달 실패 또는 빈 배열 수신 시 안전하게 백엔드로 넘겨줄 기본 지표
const DEFAULT_ALL_INDICATORS = ["SMA", "MACD", "RSI", "MFI", "OBV", "SIGMA", "ADX", "CCI", "EOM"];

export default function SimulationConsole({
    selectedIndicators = DEFAULT_ALL_INDICATORS,
    setSelectedIndicators,
    onSimulationAdvance
}: ConsoleProps): React.JSX.Element {

    // 📦 [독립 상태]: 콘솔의 리포트창과 재생 상태 내부 관리
    ...
    // 🛡️ [지표 상태 안전 검증 안전망]: Props가 null 또는 undefined로 인입되어도 안전 리스트 보장
    const safeIndicators = Array.isArray(selectedIndicators) && selectedIndicators.length > 0
        ? selectedIndicators
    ...
    // ------------------------------------------------------------
    // ⚙️ 핵심 구역: 백엔드 주머니(Map)를 열어 차트와 텍스트로 분배
    // ------------------------------------------------------------
    const handleNextDaySimulation = (): void => {
        console.log("📡 [관제탑] 자식 콘솔에서 백엔드로 전진 신호 송신 시작...");

        const targetIndicatorsParam = safeIndicators.join(',');

        axios.get<any>('http://localhost:9100/api/simulation', {
            params: {
                code: "005930",
            ...
            .then((response) => {
                ...
                console.log("📥 [도킹 응답 완료] 백엔드 응답 데이터:", data);

                // 🎯 백엔드 데이터 소진("END" 또는 "FINISHED") 신호 수신 시 자동 타이머 해제
                if (data.status === "END" || data.status === "FINISHED") {
                    setSimReport(data.message || "🏁 모든 시뮬레이션 데이터가 소모되었습니다.");

                    stopAutoPlay(); // 🎯 타이머 안전하게 해제
            ...
            .catch((error) => {
                ...
                stopAutoPlay(); // 🎯 에러 발생 시 자동 플레이 정지
            });
    };

    // ⚡ 체크박스 선택 시 토글 핸들러
    const handleIndicatorChange = (indicator: string) => {
        if (!setSelectedIndicators) return;

        setSelectedIndicators((prev) => {
            const currentList = Array.isArray(prev) ? prev : DEFAULT_ALL_INDICATORS;
            if (currentList.includes(indicator)) {
                return currentList.filter((item) => item !== indicator);
            } else {
                return [...currentList, indicator];
            }
        });
    };
    ...
    const toggleAutoPlay = (): void => {
        if (isPlaying) {
            ...
            setIsPlaying(true);
            // 🎯 기존 실행 중인 타이머가 있다면 확실히 정리 후 재시작
            if (timerRef.current) clearInterval(timerRef.current);
    ...
    useEffect(() => {
        // 🎯 브라우저 마운트(새로고침) 시 백엔드 인덱스 초기화 요청
        axios.post('http://localhost:9100/api/simulation/reset')
            .then(() => console.log("🔄 백엔드 시뮬레이션 인덱스 초기화 완료"))
    ...
    return (
        <div style={{ padding: '15px', backgroundColor: '#f8f9fa', borderRadius: '8px', border: '1px solid #dee2e6' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', fontSize: '12px', marginBottom: '15px', justifyContent: 'center', color: '#6c757d', flexWrap: 'wrap' }}>
                ...
                {/* 실시간 연동 스마트 체크박스 목록 */}
                <label style={{ cursor: 'pointer', color: safeIndicators.includes("RSI") ? '#2cc54d' : '#6c757d', fontWeight: safeIndicators.includes("RSI") ? 'bold' : 'normal' }}>
                    <input type="checkbox" checked={safeIndicators.includes("RSI")} onChange={() => handleIndicatorChange("RSI")} /> RSI(단기심리)
                </label>
                <label style={{ cursor: 'pointer', color: safeIndicators.includes("MFI") ? '#e83e8c' : '#6c757d', fontWeight: safeIndicators.includes("MFI") ? 'bold' : 'normal' }}>
                    <input type="checkbox" checked={safeIndicators.includes("MFI")} onChange={() => handleIndicatorChange("MFI")} /> MFI(자금흐름)
                </label>
                <label style={{ cursor: 'pointer', color: safeIndicators.includes("SIGMA") ? '#007bff' : '#6c757d', fontWeight: safeIndicators.includes("SIGMA") ? 'bold' : 'normal' }}>
                    <input type="checkbox" checked={safeIndicators.includes("SIGMA")} onChange={() => handleIndicatorChange("SIGMA")} /> SIGMA(변동성)
                </label>
                <label style={{ cursor: 'pointer', color: safeIndicators.includes("ADX") ? '#af5288' : '#6c757d', fontWeight: safeIndicators.includes("ADX") ? 'bold' : 'normal' }}>
                    <input type="checkbox" checked={safeIndicators.includes("ADX")} onChange={() => handleIndicatorChange("ADX")} /> ADX(매수강도)
                </label>
                {/* 🎯 [추가]: 누락되었던 CCI 체크박스 보완 */}
                <label style={{ cursor: 'pointer', color: safeIndicators.includes("CCI") ? '#17a2b8' : '#6c757d', fontWeight: safeIndicators.includes("CCI") ? 'bold' : 'normal' }}>
                    <input type="checkbox" checked={safeIndicators.includes("CCI")} onChange={() => handleIndicatorChange("CCI")} /> CCI(추세순응)
                </label>
                <label style={{ cursor: 'pointer', color: safeIndicators.includes("EOM") ? '#d97706' : '#6c757d', fontWeight: safeIndicators.includes("EOM") ? 'bold' : 'normal' }}>
                    <input type="checkbox" checked={safeIndicators.includes("EOM")} onChange={() => handleIndicatorChange("EOM")} /> EOM(용이성)
                </label>
            </div>
...
// interface ConsoleProps {
//     onNextDay: () => void; // 또는 () => Promise<void>
//     reportText: string;
