...
// 📋 1. DB 스냅샷 명세와 100% 일치하는 데이터 구조 타입 정의 (EOM 추가)
...
interface EomParams { EOM_PERIOD: number; } // 🎯 EOM 파라미터 타입 추가
...
interface IndicatorParams {
  ...
  ADX: AdxParams;
  CCI: CciParams;
  EOM: EomParams; // 🎯 EOM 그룹 추가
}

const IndicatorConfig: React.FC = () => {
  // 📊 2. stock_index_config 데이터베이스 기준 최초 초기값 세팅 (EOM 추가)
  const defaultParams: IndicatorParams = {
    ...
    SIGMA: { SIGMA_PERIOD: 20, SIGMA_MULTIPLIER: 2.0 },
    ADX: { ADX_PERIOD: 14 },
    CCI: { CCI_PERIOD: 20, CCI_SIGNAL: 9 },
    EOM: { EOM_PERIOD: 14 } // 🎯 EOM 기본값 추가
  };
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#f8f9fa', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
      ...
      {/* 2층. MACD 그룹 */}
      <div style={cardStyle}>
        <h4 style={{ margin: '0 0 8px 0', color: '#2962e7' }}>MACD (이동평균 수렴확산지수)</h4>
        <div style={badgeStyle}>
      ...
      {/* 5층. ADX, CCI, EOM 그룹 (3열 그리드 배치) */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
        {/* ADX 카드 */}
        <div style={{ ...cardStyle, marginBottom: 0 }}>
          <h4 style={{ margin: '0 0 6px 0', color: '#0f172a', fontSize: '14px' }}>ADX (추세 강도)</h4>
          <div style={{ fontSize: '12px', color: '#666', marginBottom: '8px' }}>
        ...
        {/* CCI 카드 */}
        <div style={{ ...cardStyle, marginBottom: 0 }}>
          <h4 style={{ margin: '0 0 6px 0', color: '#0f172a', fontSize: '14px' }}>CCI (상품 채널)</h4>
          <div style={{ fontSize: '12px', color: '#666', marginBottom: '8px' }}>
            최초: 20 → <span style={{ color: '#0d9488', fontWeight: 'bold' }}>현재: {currentParams.CCI.CCI_PERIOD}</span>
          </div>
        ...
        {/* 🎯 EOM 카드 (신규 추가) */}
        <div style={{ ...cardStyle, marginBottom: 0 }}>
          <h4 style={{ margin: '0 0 6px 0', color: '#0f172a', fontSize: '14px' }}>EOM (용이성 지표)</h4>
          <div style={{ fontSize: '12px', color: '#666', marginBottom: '8px' }}>
            최초: 14 → <span style={{ color: '#d97706', fontWeight: 'bold' }}>현재: {currentParams.EOM.EOM_PERIOD}</span>
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <input type="number" style={inputStyle} value={inputParams.EOM.EOM_PERIOD || ''} onChange={(e) => handleInputChange('EOM', 'EOM_PERIOD', e.target.value)} />
            <button onClick={() => handleApply('EOM')} style={btnStyle('#d97706')}>변경</button>
          </div>
        </div>
      </div>
